  ...
  <div class="wrap-container">
    <div class="wrap-header">
      ...
      <div class="header-actions">
        <el-input v-model="searchForm.spec_name" placeholder="请输入规格名称" style="width: 150px" clearable></el-input>
        <el-button round icon="Plus" type="primary" @click="createHandler">新增</el-button>
        <el-button round type="primary" icon="Search" @click="searchSpecificationList">搜索</el-button>
    ...
    <div class="wrap-table">
      <el-table :data="listData" v-loading="loading">
        ...
        <el-table-column prop="spec_code" label="规格编码" width="150">
          <template #default="{ row }">
            <el-tag type="primary">{{ row.spec_code }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="spec_name" label="规格名称" width="150"></el-table-column>
      ...
      <el-pagination
        ...
        @current-change="fetchSpecificationList"
    ...
    <el-dialog v-model="dialogFormVisible" :title="dialogTitle" width="600">
      <el-form :model="formData" label-width="100px">
        <el-form-item v-show="dialogTitle === '编辑规格'" label="规格编码">
          <el-input disabled v-model="formData.spec_code"></el-input>
        </el-form-item>
        <el-form-item label="规格名称" required>
          <el-input v-model="formData.spec_name" placeholder="请输入规格名称（如：500ml、1L）"></el-input>
        </el-form-item>
...
import { getSpecificationList, deleteSpecification, addSpecification, updateSpecification } from '@/api/specification'
...
const fetchSpecificationList = async () => {
  loading.value = true
  try {
    const response = await getSpecificationList({ ...page, ...searchForm })
...
const searchSpecificationList = () => {
  page.pageNum = 1
  fetchSpecificationList()
}

const resetSearchHandler = () => {
  searchForm.spec_name = ''
  page.pageNum = 1
  fetchSpecificationList()
}
...
const deleteHandler = async (row: any) => {
  try {
    ...
    const result = await deleteSpecification(row.id)
...
const editHandler = (row: any) => {
  ...
  formData.id = row.id
  formData.spec_code = row.spec_code
  formData.spec_name = row.spec_name
...
const addSubmitHandler = async () => {
  ...
  try {
    if (dialogTitle.value === '新增规格') {
      const res = await addSpecification(formData)
      ...
      const res = await updateSpecification(formData)
      if (res.data.code === 2000) {
